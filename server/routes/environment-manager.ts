...
environmentManagerRouter.use(authenticate, requireRole("admin"));
...
environmentManagerRouter.post("/environments", async (req, res, next) => {
  try {
    ...
    if (
      ...
      (locale !== undefined && !["es", "en", "de", "de-CH"].includes(locale)) ||
